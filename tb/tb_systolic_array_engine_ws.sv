module tb_systolic_array_engine_ws;
  ...
  logic [COLS*ACC_W-1:0]        acc_rd_bram_data_s0;
  ...
  always_ff @(posedge aclk_i) begin
    if (!aresetn_i) begin
      ...
      if (acc_rd_bram_en_o) begin
        acc_rd_bram_data_s0 <= acc_mem[acc_rd_bram_addr_o];
      end
      act_bram_data_i    <= act_bram_data_s0;
      weight_bram_data_i <= weight_bram_data_s0;
      acc_rd_bram_data_i <= acc_rd_bram_data_s0;
  ...
  initial begin
    ...
    // A = [[1, 2], [3, 4]], WS layout: addr = k_tile * M + m.
    act_mem[10] = pack_act(1, 2);
    act_mem[11] = pack_act(3, 4);

    // B = [[5, 6], [7, 8]], WS weight layout: k row words inside each n/k tile.
    ...
    wait_done();
    check_acc_word(140, 19, 22);
    check_acc_word(141, 43, 50);

    $display("tb_systolic_array_engine_ws PASS");
    $finish;
  end
endmodule
