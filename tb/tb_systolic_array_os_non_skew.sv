module tb_systolic_array_os_non_skew;
  ...
  logic                aclk_i;
  logic                aresetn_i;
  logic [   ACT_W-1:0] act_i         [ROWS];
  logic [WEIGHT_W-1:0] weight_i      [COLS];
  logic                act_valid_i   [ROWS];
  logic                weight_valid_i[COLS];
  logic                acc_clear_i;
  logic [   ACC_W-1:0] acc_o         [ROWS][COLS];

  int unsigned a_mat [ROWS][K];
  int unsigned b_mat [K][COLS];
  int unsigned exp_c [ROWS][COLS];

  systolic_array_os_non_skew #(
      .ROWS(ROWS),
      .COLS(COLS),
      .ACT_W(ACT_W),
      .WEIGHT_W(WEIGHT_W),
  ...
  initial begin
    ...
    aresetn_i      = 1'b0;
    act_valid_i    = '{default: 1'b0};
    weight_valid_i = '{default: 1'b0};
    acc_clear_i    = 1'b0;
    drive_zero();
    ...
    for (t = 0; t < K + ROWS + COLS; t++) begin
      drive_zero();

      for (r = 0; r < ROWS; r++) begin
        idx = t - r;
        if ((idx >= 0) && (idx < K)) begin
          act_i[r] = a_mat[r][idx][ACT_W-1:0];
        end
      end
      ...
      for (c = 0; c < COLS; c++) begin
        idx = t - c;
        if ((idx >= 0) && (idx < K)) begin
          weight_i[c] = b_mat[idx][c][WEIGHT_W-1:0];
        end
      end

      act_valid_i    = '{default: 1'b1};
      weight_valid_i = '{default: 1'b1};
      tick();
    end
    ...
    check_acc("matrix multiply");

    acc_clear_i = 1'b1;
    tick();
    acc_clear_i = 1'b0;
    repeat (COLS) tick();
    check_zero("clear after stream");

    $display("tb_systolic_array_os PASS");
    $finish;
  end
endmodule
